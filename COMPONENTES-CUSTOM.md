...
| `_RaiaDoQuadro.vue` | tarefas-rapidas | O `EnKanbanBoard` não expõe slot de coluna, então não há onde pendurar o totalizador, o menu da raia nem o "ver mais" | `EnKanbanBoard` como referência de comportamento (arraste, agrupamento, menu de contexto), remontado com `UButton`, `UDropdownMenu` e `USkeleton` | Rodapé por coluna (`slot #column-footer` ou prop `columnSummary`), `columnLimit`, `collapsedColumns`, `pageSizePerColumn` com evento `load-more` e `cardDensity`. A lista completa está no `DECISOES.md` do protótipo |
