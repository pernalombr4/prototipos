...
| `_BarraDeSelecao.vue` | padrao-dos-campos (seleção em massa na tabela) | Não é falta do Nuxt UI: a barra é composta de `UIcon`, `UButton` e `UModal`. Está aqui porque é **a mesma barra** do protótipo `tarefas-rapidas`, e o dev precisa saber que são uma peça só, não duas parecidas | O mesmo desenho já decidido em `tarefas-rapidas/_BarraDeSelecao.vue`, com as ações desta tela | Um componente de barra de ação em massa no SDK, recebendo a contagem e a lista de ações, para as telas não divergirem. **Duplicação conhecida:** enquanto ele não existe, há duas cópias no repositório, uma por protótipo |
