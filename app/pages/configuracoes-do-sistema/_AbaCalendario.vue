...
const nova = reactive<{ data: string, nome: string, tipo: Ocorrencia['tipo'], contaComoUtil: boolean }>({
  data: '2026-09-30',
  nome: '',
  tipo: 'Evento interno',
  contaComoUtil: true,
...
const celulas = computed<Celula[]>(() => {
  ...
  return Array.from({ length: 42 }, (_, i) => {
    ...
    const ocorrencia = form.calendario.ocorrenciasHabilitadas
      ? form.calendario.ocorrencias.find(o => o.data === data)
      : undefined
...
<template>
  <div class="space-y-5">
      ...
      <div v-if="form.calendario.ocorrenciasHabilitadas">
        ...
        >
          <li
            ...
            class="group flex items-center gap-3 px-3 py-2 transition-colors hover:bg-elevated"
          >
            <span class="w-20 shrink-0 text-sm tabular-nums text-muted">{{ formatarData(o.data) }}</span>
    ...
    <Secao
      ...
      resumo="A prévia do que as regras acima produzem. Nada aqui se edita: é o resultado."
      :doc="documentacao.calendario"
    ...
    >
      <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <p class="text-sm text-muted">
          ...
          {{ contar(resumoDoMes.ocorrencias, 'ocorrência', 'ocorrências') }}
        </p>
      ...
      <div class="overflow-hidden rounded-lg border border-default">
        ...
        <div class="grid grid-cols-7">
          <UTooltip
            ...
            :key="c.data"
            :text="c.motivo"
            :disabled="c.util && !c.ocorrencia"
          >
            <div
              class="relative flex h-16 w-full flex-col gap-0.5 border-b border-r border-default p-1.5 text-left transition-colors last:border-r-0"
              :class="[
                !c.doMes && 'opacity-35',
                c.feriado
                  ? 'bg-warning/10'
      ...
      <template #body>
        <div class="space-y-4">
          <UFormField label="Data">
            <UInput v-model="nova.data" type="date" class="w-full sm:max-w-xs" />
          </UFormField>
