...
/**
 * A casca da tela NOVA de itens, copiada do develop.
 *
 * ⚠️ NADA AQUI É PROPOSTA. Menu lateral, barra do topo, trilha, abas e o selo
 * do Lab são a reprodução do que o produto já tem, para o padrão dos campos ser
 * avaliado no lugar onde ele vive. Se alguma coisa desta casca estiver
 * diferente do develop, é defeito de cópia, não sugestão de mudança.
 *
 * Referência: `https://develop.enspace.io/workspaces/teste-ux/types/leve?view=default`,
 * com as cinco funcionalidades do Lab ligadas, visitada em 22/09/2026. O que
 * foi copiado, item por item, está no BRIEFING.md, em "A casca da tela".
 *
 * Por que não é o `EnLayout` do SDK: o `EnLayout` publica a ESTRUTURA da casca
 * com slots vazios, não o menu do produto. Como o que se avalia é "a tela igual
 * à de hoje", a casca foi remontada com as mesmas medidas, ordem e ícones.
 * Na implementação isto é o `EnLayout` com os slots preenchidos.
 */
...
const membro: ItemDeMenu[] = [
  ...
  { chave: 'spaceflows', icone: 'i-lucide-workflow' },
  {
    ...
    expansivel: true,
    filhos: [
      { chave: 'vitrine', icone: 'i-lucide-file-text' },
      { chave: 'todos', icone: 'i-lucide-plus', ativo: true },
    ],
  },
  { chave: 'tarefas', icone: 'i-lucide-file-text', expansivel: true },
...
/** O produto abre "Categorias" porque a tela de dentro dela está aberta. */
const abertos = ref<string[]>(['categorias'])
  ...
  <div class="flex h-dvh overflow-hidden bg-default">
    ...
    >
      ...
      <nav class="min-h-0 flex-1 overflow-y-auto pb-4">
        <template
          v-for="(grupo, i) in [
            { titulo: t.casca.membro, itens: membro },
            { titulo: t.casca.configuracoes, itens: configuracoes },
            { titulo: t.casca.ajuda, itens: ajuda },
          ]"
          :key="i"
        >
          <p v-if="!menuRecolhido" class="px-4 pb-1 pt-4 text-xs font-medium text-muted">
            {{ grupo.titulo }}
          </p>
    ...
    <div class="flex min-w-0 flex-1 flex-col">
      <header class="flex h-11 shrink-0 items-center gap-1 border-b border-default px-2">
        ...
        <div class="mx-2 flex min-w-0 flex-1 items-center gap-2 rounded-md bg-elevated/60 px-3 py-1.5">
          ...
          <nav class="flex min-w-0 items-center gap-1.5 text-xs" :aria-label="t.casca.trilha">
            ...
            <span class="truncate text-toned">{{ t.trilhaCategorias }}</span>
            <UIcon name="i-lucide-chevron-right" class="size-3 shrink-0 text-dimmed" />
            <span class="truncate font-medium text-highlighted">{{ categoria }}</span>
          </nav>
      ...
      <div class="relative flex min-h-0 flex-1 flex-col">
        <!-- O selo do Lab, que a tela nova mostra no canto direito -->
        <UTooltip :text="t.casca.lab">
          <span class="absolute right-3 top-2 z-20 flex size-7 items-center justify-center rounded-md bg-error/10 text-error">
